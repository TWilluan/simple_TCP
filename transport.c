...
void wait_for_ACK(mysocket_t sd, context_t *ctx)
{
    ...
    if (packet->th_flags == TH_ACK)
    {
        ...
        if (ctx->connection_state == FIN_SENDER_ACK)
            ctx->connection_state = FIN_READER;
        if (ctx->connection_state == FIN_READER_ACK)
...
void app_data_event(mysocket_t sd, context_t *ctx)
{
    unsigned int length = (STCP_MSS < ctx->window_size) ? STCP_MSS : ctx->window_size;
...
void network_data_event(mysocket_t sd, context_t *ctx)
{
    ...
    ssize_t bytes;
    if ((bytes = stcp_network_recv(sd, payload, STCP_MSS)) < sizeof(STCPHeader))
    {
        free(ctx);
        errno = ECONNREFUSED;
        return;
    ...
    if (isFin)
    {
        ...
        {
            ctx->connection_state = FIN_SENDER_ACK;
            send_ACK(sd, ctx);
        }
        
        if (ctx->connection_state == FIN_READER)
        {
            ctx->connection_state = FIN_READER_ACK;
            send_ACK(sd, ctx);
        }
    ...
    {
        send_DATA_app(sd, ctx, payload, bytes);
        send_ACK(sd, ctx);
    }
}
