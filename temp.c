...
typedef struct
{
    ...
    unsigned int send_seq_num; // next sequence number to send
...
void transport_init(mysocket_t sd, bool_t is_active)
{
     ...
     */
    connection_setup(sd, ctx, is_active);
...
bool send_SYN_ACK(mysocket_t sd, context_t *ctx)
{
    STCPHeader *packet = create_SYN_ACK_packet(ctx->send_seq_num, ctx->recv_seq_num + 1);
    ctx->send_seq_num++;
...
bool send_ACK(mysocket_t sd, context_t *ctx)
{
    STCPHeader *packet = create_ACK_packet(ctx->send_seq_num, ctx->recv_seq_num + 1);
    // ctx->send_seq_num++; ??
...
void wait_for_SYN_ACK(mysocket_t sd, context_t *ctx)
{
    ...
    ssize_t bytes;
    if ((bytes = stcp_network_recv(sd, buffer, STCP_MSS)) < 0)
    {
        free(ctx);
        errno = ECONNREFUSED;
    }
    ...
    {
        ctx->send_seq_num = ntohl(packet->th_seq);
...
bool send_FIN(mysocket_t sd, context_t *ctx)
{
    STCPHeader *packet = create_FIN_packet(ctx->send_seq_num, ctx->recv_seq_num + 1);
    ctx->send_seq_num++;

    ssize_t bytes;
    if ((bytes = stcp_network_send(sd, packet, sizeof(STCPHeader), NULL)) > 0)
    {
        ctx->connection_state = FIN_1_SENT;
        wait_for_ACK(sd, ctx);
        free(packet);
        return true;
    }
...
bool send_DATA_network(mysocket_t sd, context_t *ctx,
                       char *payload, size_t payload_length)
{
    STCPHeader *packet = create_DATA_packet(ctx->send_seq_num,
